...
from models.svdxt_featureflow_forward_controlnet_s2d_fixcmp_norefine import DualFlowControlNet_traj as FlowControlNet
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = latent_model_input.to(device=device, dtype=latents.dtype)
...
class DualFlowControlNetPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        controlnet_condition: [torch.FloatTensor] = None,  # PIL
        controlnet_flow: [torch.FloatTensor] = None,  # [1, 13, 2, h, w]
        controlnet_depth: [torch.FloatTensor] = None,  # [1, 13, 2, h, w]
        ...
        # dual condition for for the controlnet_flow, if not, use the line 393
        controlnet_flow = torch.cat([controlnet_flow] * 2) if do_classifier_free_guidance else latents
        controlnet_flow = controlnet_flow.to(device, latents.dtype)
