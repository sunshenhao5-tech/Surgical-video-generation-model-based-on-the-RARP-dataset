...
def cuda_launch(strKey:str):
    ...
    include_dir = os.environ['CUDA_HOME'] + '/include'
    raw_mod = cupy.RawModule(
        code=objCudacache[strKey]['strKernel'],
        options=('-I ' + include_dir,),
        backend='nvcc',
    )
    return raw_mod.get_function(objCudacache[strKey]['strFunction'])
# end
...
class softsplat_func(torch.autograd.Function):
    ...
    def forward(self, tenIn, tenFlow):
        # Pure PyTorch bilinear splatting (forward warping via scatter_add)
        N, C, H, W = tenIn.shape
        tenOut = tenIn.new_zeros([N, C, H, W])

        # Source pixel grid
        grid_y = torch.arange(H, device=tenIn.device, dtype=torch.float32)
        grid_x = torch.arange(W, device=tenIn.device, dtype=torch.float32)
        gy, gx = torch.meshgrid(grid_y, grid_x, indexing='ij')  # (H, W)
        gy = gy.unsqueeze(0).expand(N, -1, -1)  # (N, H, W)
        gx = gx.unsqueeze(0).expand(N, -1, -1)

        # Target positions
        fltX = gx + tenFlow[:, 0]  # (N, H, W)
        fltY = gy + tenFlow[:, 1]

        # Filter out invalid positions
        valid = torch.isfinite(fltX) & torch.isfinite(fltY)

        # Bilinear splat: distribute each source pixel to 4 neighbors
        for dy, dx in [(0, 0), (0, 1), (1, 0), (1, 1)]:
            tgtX = (fltX.floor() + dx).long()  # (N, H, W)
            tgtY = (fltY.floor() + dy).long()
            # Bilinear weight
            wx = 1.0 - (fltX - (fltX.floor() + dx)).abs()
            wy = 1.0 - (fltY - (fltY.floor() + dy)).abs()
            weight = wx * wy  # (N, H, W)

            # Mask out-of-bounds and invalid
            mask = valid & (tgtX >= 0) & (tgtX < W) & (tgtY >= 0) & (tgtY < H)

            # Flatten spatial dims for scatter
            idx = (tgtY.clamp(0, H-1) * W + tgtX.clamp(0, W-1))  # (N, H, W)
            idx = idx * mask.long()  # zero out invalid (will be masked anyway)

            for n in range(N):
                m = mask[n].reshape(-1)         # (H*W,)
                w = (weight[n] * mask[n].float()).reshape(-1)  # (H*W,)
                src = tenIn[n].reshape(C, -1)   # (C, H*W)
                i = idx[n].reshape(-1)          # (H*W,)
                # scatter: tenOut[n, :, tgtY, tgtX] += weight * tenIn[n, :, srcY, srcX]
                tenOut[n].reshape(C, -1).scatter_add_(1, i.unsqueeze(0).expand(C, -1), src * w.unsqueeze(0))
